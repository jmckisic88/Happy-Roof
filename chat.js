...
(function () {
  ...
  function render() {
    ...
    messages.forEach(m => {
      ...
      bubble.style.cssText = m.from === 'user'
        ? 'background:#7C3AED;color:#fff;border-radius:14px 14px 4px 14px;padding:.625rem .875rem;max-width:82%;font-size:.88rem;line-height:1.55'
        : 'background:#1A1A1A;color:#E0E0E0;border-radius:14px 14px 14px 4px;padding:.625rem .875rem;max-width:88%;font-size:.88rem;line-height:1.55;border:1px solid #252525';
      ...
      if (m.quick && m.quick.length) {
        ...
        m.quick.forEach(q => {
          ...
          btn.textContent = q;
          btn.style.cssText = 'background:transparent;border:1px solid #2E2E2E;border-radius:100px;color:#9D5CF6;font-size:.75rem;padding:.3rem .75rem;cursor:pointer;transition:background .15s,border-color .15s;font-family:inherit';
          btn.onmouseenter = () => { btn.style.background='rgba(124,58,237,.1)'; btn.style.borderColor='rgba(124,58,237,.4)'; };
  ...
  function init() {
    ...
    win.style.cssText = [
      ...
      'background:#0D0D0D;border:1px solid #222;border-radius:20px',
    ...
    header.innerHTML = `
      <div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#A855F7,#7C3AED);display:flex;align-items:center;justify-content:center;flex-shrink:0">
      ...
      <div style="flex:1;min-width:0">
        <p style="font-family:'Barlow Condensed',sans-serif;font-weight:800;font-size:1rem;color:#F5F5F5;letter-spacing:.01em;line-height:1.1">Happy Roof Assistant</p>
        <p style="font-size:.72rem;color:#0FBFBF;margin-top:.05rem">● Online · Typically replies instantly</p>
      </div>
      <button onclick="document.getElementById('hr-chat-window').style.display='none';document.getElementById('hr-chat-fab').innerHTML='<svg width=\\'22\\' height=\\'22\\' viewBox=\\'0 0 24 24\\' fill=\\'none\\'><path d=\\'M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z\\' stroke=\\'currentColor\\' stroke-width=\\'2\\' stroke-linejoin=\\'round\\'/></svg>'" style="background:transparent;border:none;cursor:pointer;color:#555;padding:.25rem;border-radius:6px;display:flex;align-items:center;transition:color .15s" onmouseenter="this.style.color='#F5F5F5'" onmouseleave="this.style.color='#555'">
    ...
    inputWrap.style.cssText = 'padding:.75rem;border-top:1px solid #1A1A1A;display:flex;gap:.5rem;flex-shrink:0;background:#0D0D0D';

    const inp = document.createElement('input');
    inp.id = 'hr-chat-input';
    inp.type = 'text';
    inp.placeholder = 'Ask about roofing…';
    inp.style.cssText = 'flex:1;background:#141414;border:1px solid #252525;border-radius:10px;padding:.6rem .875rem;color:#F5F5F5;font-size:.875rem;font-family:inherit;outline:none;transition:border-color .18s;min-width:0';
    inp.onfocus = () => inp.style.borderColor = 'rgba(124,58,237,.5)';
    inp.onblur = () => inp.style.borderColor = '#252525';
    inp.onkeydown = e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMsg(); } };

    const sendBtn = document.createElement('button');
    sendBtn.style.cssText = 'background:#7C3AED;border:none;border-radius:10px;width:38px;height:38px;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;transition:background .18s,transform .15s';
    ...
    sendBtn.onmouseleave = () => { sendBtn.style.background='#7C3AED'; sendBtn.style.transform='scale(1)'; };
    ...
    fab.style.cssText = [
      ...
      'box-shadow:0 8px 28px -4px rgba(124,58,237,.6),0 4px 12px -4px rgba(0,0,0,.4)',
    ...
    fab.onmouseenter = () => { fab.style.transform='scale(1.08)'; fab.style.boxShadow='0 12px 36px -4px rgba(124,58,237,.7),0 4px 12px -4px rgba(0,0,0,.4)'; };
    fab.onmouseleave = () => { fab.style.transform='scale(1)'; fab.style.boxShadow='0 8px 28px -4px rgba(124,58,237,.6),0 4px 12px -4px rgba(0,0,0,.4)'; };

    // Notification badge
    const badge = document.createElement('span');
    badge.id = 'hr-chat-badge';
    badge.style.cssText = 'position:absolute;top:-4px;right:-4px;width:18px;height:18px;background:#0FBFBF;border-radius:50%;border:2px solid #0D0D0D;display:flex;align-items:center;justify-content:center;font-size:.6rem;font-weight:700;color:#0D0D0D';
    badge.textContent = '1';
  ...
  function initMobileBar() {
    ...
    bar.innerHTML = `
      <a href="tel:8135957663" style="display:flex;align-items:center;justify-content:center;gap:.45rem;padding:.75rem;background:#141414;border:1px solid #222;border-radius:10px;color:#0FBFBF;font-family:'Barlow Condensed',sans-serif;font-weight:800;font-size:1rem;letter-spacing:.03em;text-decoration:none">
      ...
      <a href="contact.html" style="display:flex;align-items:center;justify-content:center;gap:.45rem;padding:.75rem;background:#7C3AED;border-radius:10px;color:#fff;font-family:'Barlow Condensed',sans-serif;font-weight:800;font-size:1rem;letter-spacing:.03em;text-decoration:none;box-shadow:0 4px 16px -4px rgba(124,58,237,.55)">
